...
5. **选择打包模式**
   - 输入 `1` - 完整版 - 文件夹模式（包含所有OCR引擎）
   - 输入 `2` - **核心版 - 文件夹模式（推荐，仅250MB）**
   - 输入 `3` - 单文件模式
   - 输入 `4` - 清理缓存和临时文件
   - 输入 `5` - 退出
...
| 使用场景 | 推荐模式 | 原因 |
|---------|---------|------|
| 正式发布 | **核心版 + 文件夹模式** | 体积小，启动快，用户按需下载引擎 |
| 快速测试 | 核心版 + 文件夹模式 | 构建快，便于调试 |
| 演示/试用 | 完整版 + 文件夹模式 | 开箱即用，无需下载 |
| 企业内部 | 核心版 + 文件夹模式 | 便于维护和更新 |
| 离线环境 | 完整版 + 文件夹模式 | 包含所有必需组件 |

**建议**: 对于大多数用户，推荐使用**核心版 + 文件夹模式**，然后使用 7-Zip 压缩成 `.7z` 文件进行分发。用户首次启动时可在程序内下载需要的OCR引擎。
...
| 打包模式 | 预期大小 | 说明 |
|---------|---------|------|
| **核心版（不含OCR引擎）** | ~250 MB | **🆕 推荐方式**，OCR引擎可选下载 |
| **完整版（含RapidOCR）** | ~600 MB | 包含RapidOCR引擎 |
| **完整版（含PaddleOCR）** | ~1.2 GB | 包含PaddleOCR引擎 |
| **核心版（压缩后）** | ~120-150 MB | 使用 7-Zip 压缩 |
| **完整版（压缩后）** | ~300-350 MB | 使用 7-Zip 压缩 |
